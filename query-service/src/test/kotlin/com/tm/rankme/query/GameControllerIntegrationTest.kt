...
internal class GameControllerIntegrationTest {
    ...
    @Test
    internal fun `Should return games page`() {
        // given
        ...
        // when
        val result = mvc.get("/games?leagueId=$leagueId&first=2&after=$afterCursor")
        // then
        result.andExpect {
            status { isOk() }
    ...
    internal fun `Should return empty games page`() {
        ...
        // when
        val result = mvc.get("/games?leagueId=$leagueId&first=3")
        // then
        result.andExpect {
            status { isOk() }
